describe('Validar o endpoint /species', () => {
  it('Validar o método GET', () => {
    cy.request('/species/').then(response => {
      ...
      expect(response.body.next).to.include('https://swapi.dev/api/species/?page=')
      ...
      expect(response.body.results[0].homeworld).to.include('https://swapi.dev/api/planets/')
      expect(response.body.results[0].language).to.be.a('string')
      expect(response.body.results[0].people).to.be.an('array')
      expect(response.body.results[0].people[0]).to.include('https://swapi.dev/api/people/')
      ...
      expect(response.body.results[0].url).to.include('https://swapi.dev/api/species/')
    })
  })
})